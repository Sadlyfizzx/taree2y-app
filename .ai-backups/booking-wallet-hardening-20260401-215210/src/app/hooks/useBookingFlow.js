...
function hasValidBookingResult(result) {
  return Boolean(result?.ok && result?.booking && result?.invoice);
}

function hasValidHoldResult(result) {
  return Boolean(result?.ok && getHoldExpiry(result));
}
...
export function useBookingFlow({
  ...
  const handleSearch = useCallback(async (predefinedParams = null) => {
    const params = { ...searchParams, ...(predefinedParams || {}) };
  ...
  const createBookingForTrip = useCallback(async ({ trip, seatNumbers, passengers, promoCode, promoDiscountAmount: _promoDiscountAmount = 0, hasLuggage, needsAccess }) => {
    ...
    if (!hasValidBookingResult(result)) {
      log.error('booking_contract_invalid', {
        tripInstanceId: trip.instanceId,
        hasBooking: Boolean(result?.booking),
        hasInvoice: Boolean(result?.invoice),
      });
      try {
        await refreshCloudState({ silent: true, force: true });
      } catch (refreshError) {
        log.warn('booking_contract_invalid_refresh_failed', {
          tripInstanceId: trip.instanceId,
          error: refreshError,
        });
      }
      return {
        ok: false,
        code: 'booking_contract_invalid',
        message: 'استجابة تأكيد الحجز من السيرفر ناقصة. حدّث الحالة وحاول تاني.',
      };
    }
  ...
  useEffect(() => {
    ...
    const restore = async () => {
      try {
        ...
        if (!savedTrip?.instanceId) {
          clearBookingFlowDraft();
          setSelectedTrip(savedTrip);
          setSelectedSeats([]);
          navigateTo('search');
          showToast('الرابط المحفوظ للحجز لم يعد صالحًا لأن الرحلة لم تعد مرتبطة بعقد حجز معتمد من السيرفر.', 'warning');
          return;
        }
  ...
  useEffect(() => {
    ...
    const api = {
      ...
      async goCheckout() { const result = await commitSeatSelection(qaStateRef.current.selectedTrip, qaStateRef.current.selectedSeats); await wait(160); return { ...this.snapshot(), hold: result }; },
      ...
      async smoke() {
        ...
        const seats = await this.selectFirstAvailableSeats(1);
        const checkout = await this.goCheckout();
        const invoice = await this.completeBooking();
        await wait(150);
        return {
          ok: true,
          seats,
