...
export default function PublicWalletTopUpView() {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const confirmedPaid = Boolean(
        result?.ok &&
          nextRequest?.requestId &&
          (result?.alreadyPaid || nextRequest?.isPaid || nextRequest?.status === 'paid')
      );
      ...
      if (!confirmedPaid) {
        setStatus('error');
        setMessage(result?.message || 'تعذر تأكيد الشحن لأن استجابة السيرفر ناقصة أو غير نهائية.');
        return;
      }
