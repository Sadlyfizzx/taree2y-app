...
export function AppSurface({ children, className = '', tone = 'default' }) {
  return (
    <section
      className={cx(
        'rounded-[28px] border border-slate-200/80 bg-white shadow-[0_18px_40px_-28px_rgba(16,35,63,0.24)] transition-[transform,box-shadow,border-color,background-color] duration-300 dark:border-slate-800 dark:bg-slate-900',
    ...
    >
      ...
      {!collapsed ? (
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 text-[11px] font-bold text-slate-400 dark:text-slate-500">وصول أسرع</p>
        </div>
      ) : null}
    </button>
  );
...
export function BottomNavItem({ icon, label, active, onClick }) {
  return (
    <button
      ...
      onClick={onClick}
      className={cx(
        'group relative flex min-h-[60px] flex-1 flex-col items-center justify-center gap-1 rounded-[22px] px-2 text-center transition-all duration-200',
