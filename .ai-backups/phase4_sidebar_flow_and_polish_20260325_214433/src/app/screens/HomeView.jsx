...
import { ArrowRightLeft, Award, Bell, BookOpen, Calendar, Crown, MapPin, Search, Ticket, Users, Wallet as WalletIcon } from 'lucide-react';
...
function UtilityCard({ icon: Icon, label, value, note, onClick, tone, mobileOnly = false }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`rounded-[26px] border border-slate-200 bg-white p-4 text-right shadow-sm transition-all duration-200 hover:-translate-y-[1px] hover:border-indigo-200 hover:shadow-md dark:border-slate-800 dark:bg-slate-900 dark:hover:border-indigo-800 ${mobileOnly ? 'md:hidden' : ''}`}
    >
...
export default function HomeView({
  ...
  return (
    <div className="space-y-5 md:space-y-6">
      <PageHeading
        eyebrow="الرئيسية"
        title="احجز بسرعة من غير دوشة"
        subtitle="ابدأ الحجز من هنا، وكل قسم تاني له مكانه الواضح داخل التطبيق."
      />
      ...
      {hasUpcomingTrip ? (
        <AppSurface className="overflow-hidden p-4 sm:p-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <p className="text-xs font-black tracking-[0.16em] text-slate-400 dark:text-slate-500">رحلتك القادمة</p>
      ...
      <div className="grid gap-4 md:hidden">
        <AppSurface className="overflow-hidden p-0">
          <div className="bg-[linear-gradient(135deg,#10233f_0%,#163c98_58%,#2156d9_100%)] p-5 text-white">
            ...
            <p className="mt-2 text-sm font-bold leading-6 text-white/80">اختار خط الرحلة واليوم وعدد الركاب، والباقي هنرتبه لك خطوة بخطوة.</p>
          </div>
          ...
          <div className="space-y-4 p-4">
            ...
            <div className="grid grid-cols-[1fr_auto] gap-3">
              ...
              <button type="button" onClick={handleSwap} className="mt-[30px] grid h-14 w-14 place-items-center rounded-[22px] border border-slate-200 bg-slate-50 text-slate-600 transition hover:border-indigo-200 hover:text-indigo-700 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <UtilityCard icon={Bell} label="التنبيهات" value={unreadCount ? `${unreadCount} جديد` : 'مفيش جديد'} note="راجع آخر التنبيهات" onClick={() => openModal('notifications')} tone="bg-rose-50 text-rose-700 dark:bg-rose-900/20 dark:text-rose-300" />
        </div>
      </div>

      <div className="hidden gap-5 md:grid xl:grid-cols-[1.15fr_0.85fr]">
        <AppSurface className="p-5 sm:p-6">
          <SectionHeader title="احجز رحلة جديدة" subtitle="من هنا تبدأ البحث، ثم تتكمل الخطوات صفحة بصفحة بشكل واضح." />

          {isFirstTimeUser ? (
            <div className="mt-5">
              <InlineNotice tone="info" title="أول مرة تستخدم طريقي؟" text="ابدأ بالحجز من هنا، ولو احتجت شرح سريع افتح الدليل أو مركز المساعدة." actionLabel="فتح الدليل" onAction={openGuide} />
            </div>
          ) : null}
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-1">
          <UtilityCard icon={Ticket} label="رحلاتي" value={hasUpcomingTrip ? 'رحلة قادمة' : 'إدارة الحجوزات'} note="لو عايز النسخة الكاملة من لوحة رحلتك، ستجدها هناك." onClick={onOpenBookings} tone="bg-indigo-50 text-indigo-700 dark:bg-indigo-900/20 dark:text-indigo-300" />
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-1">
            <UtilityCard icon={Ticket} label="التذاكر" value="فتح سريع" note="كل تذاكرك في صفحة مستقلة للوصول الأسرع وقت السفر." onClick={onOpenTickets} tone="bg-emerald-50 text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-300" />
            <UtilityCard icon={WalletIcon} label="المحفظة" value={`${wallet} ج.م`} note="الرصيد وحركة الفلوس في صفحة مخصصة وواضحة." onClick={onOpenWallet} tone="bg-teal-50 text-teal-700 dark:bg-teal-900/20 dark:text-teal-300" />
            <UtilityCard icon={Award} label="المكافآت" value={`${points} نقطة`} note="النقاط والدعوات والعروض معًا بدل ما تكون متفرقة." onClick={onOpenRewards} tone="bg-amber-50 text-amber-700 dark:bg-amber-900/20 dark:text-amber-300" />
            <UtilityCard icon={Crown} label="الاشتراكات" value={subscriptionLabel} note="الباقات لها صفحة مستقلة بدل ما تفضل مدفونة." onClick={onOpenSubscriptions} tone="bg-purple-50 text-purple-700 dark:bg-purple-900/20 dark:text-purple-300" />
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <UtilityCard icon={BookOpen} label="الدليل" value="شرح سريع" note="افتح الدليل لو حابب تراجع الخطوات الأساسية." onClick={openGuide} tone="bg-emerald-50 text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-300" />
        <UtilityCard icon={Bell} label="التنبيهات" value={unreadCount ? `${unreadCount} جديد` : 'مفيش جديد'} note="راجع آخر التنبيهات والإشعارات من هنا." onClick={() => openModal('notifications')} tone="bg-rose-50 text-rose-700 dark:bg-rose-900/20 dark:text-rose-300" />
        <UtilityCard icon={WalletIcon} label="المحفظة" value={`${wallet} ج.م`} note="لما تحتاج فلوسك أو حركة الحساب، هتلاقيها هناك." onClick={onOpenWallet} tone="bg-teal-50 text-teal-700 dark:bg-teal-900/20 dark:text-teal-300" mobileOnly />
      </div>
    </div>
  );
}
