...
import { Award, Copy, Crown, Gift, ShieldCheck, Sparkles } from 'lucide-react';
import ReferralCard from '../components/ui/ReferralCard';
import { AppSurface, MetaChip, PageHeading, PrimaryButton, SecondaryButton, SectionHeader } from '../components/ui/AppPrimitives';
...
export default function RewardsView({
  ...
  return (
    <div className="space-y-5">
      <PageHeading
        eyebrow="المكافآت"
        title="المكافآت والدعوات والعروض"
        subtitle="كل ما يخص القيمة الإضافية في صفحة واحدة واضحة بدل ما يكون متفرق في التطبيق."
      />

      <div className="grid gap-5 xl:grid-cols-[1.08fr_0.92fr]">
        <section className="relative overflow-hidden rounded-[36px] bg-[linear-gradient(135deg,#10233f_0%,#163c98_48%,#2156d9_100%)] p-5 text-white shadow-[0_30px_60px_-36px_rgba(16,35,63,0.6)] sm:p-6">
          ...
          <div className="relative z-10">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-3 text-4xl font-black sm:text-5xl">{pointsValue}</p>
                <p className="mt-2 text-sm font-bold text-white/80">
                  {isGold ? 'أنت بالفعل في المستوى الذهبي.' : `فاضلك ${Math.max(0, 1000 - pointsValue)} نقطة للوصول للمستوى الذهبي.`}
                </p>
              </div>
              <div className="flex flex-wrap gap-2">
                <MetaChip label={isGold ? 'عضوية ذهبية' : 'عضوية أساسية'} tone="brand" className="border-white/15 bg-white/10 text-white" />
        ...
        <AppSurface className="p-5 sm:p-6">
          <SectionHeader title="كيف تستفيد أسرع؟" subtitle="ثلاث طرق واضحة للاستفادة من الصفحة دي." />
          <div className="mt-5 grid gap-3 sm:grid-cols-3 xl:grid-cols-1">
            {[
              { title: 'استبدال النقاط', text: 'كل 500 نقطة تقدر تتحول لرصيد في المحفظة.', icon: ShieldCheck, tone: 'bg-emerald-50 text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-300' },
              { title: 'الدعوات', text: 'شارك كودك أو استخدم كود دعوة قبل أول حجز فعلي.', icon: Gift, tone: 'bg-indigo-50 text-indigo-700 dark:bg-indigo-900/20 dark:text-indigo-300' },
              { title: 'العروض', text: 'لو فيه عرض مناسب ليك هتلاقيه هنا مباشرة.', icon: Sparkles, tone: 'bg-amber-50 text-amber-700 dark:bg-amber-900/20 dark:text-amber-300' },
            ].map((item) => (
      ...
      <ReferralCard summary={referralSummary} onRefresh={refreshReferralSummary} onApplyCode={applyReferralCode} showToast={showToast} />

      <AppSurface className="p-5 sm:p-6">
        <SectionHeader title="العروض الحالية" subtitle="العروض اللي تهمك فعلًا بدل ما تكون متكدسة في الرئيسية." />
        {promoHighlights.length === 0 ? (
          <div className="mt-5">
            <InlineNotice tone="neutral" title="لا توجد عروض الآن" text="لو ظهر عرض مناسب للمسار أو للحساب، هتلاقيه هنا مباشرة." />
          </div>
        ) : (
          <div className="mt-5 grid gap-4 lg:grid-cols-3">
            {promoHighlights.map((offer) => (
              <div key={offer.code || offer.title} className="overflow-hidden rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-xs font-black tracking-[0.16em] text-slate-400 dark:text-slate-500">{offer.code ? `كود ${offer.code}` : 'عرض متاح'}</p>
                ...
                <div className="mt-5 flex flex-col gap-2 sm:flex-row">
                  ...
                  {offer.routeParams ? (
                    <SecondaryButton onClick={() => onPromoSearch?.(offer.routeParams)} className="flex-1">
                      افتح المسار
                    </SecondaryButton>
                  ) : null}
                </div>
