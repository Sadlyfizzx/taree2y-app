...
const getHeaderContent = (activeView, activeTab) => {
  if (activeView === 'search') return { title: 'نتائج البحث', subtitle: 'اختَر الرحلة المناسبة واستكمل الحجز' };
...
export default function UserApp({
  ...
  useEffect(() => {
    ...
    const loadOffer = async () => {
      ...
      if (!offer.popupEnabled || !profile?.onboarding_completed_at) {
        return;
      }
  ...
  const handleSearch = async (predefinedParams = null) => {
    ...
    try {
      ...
      if (hasFallbackSource || hasNonAuthoritativeTrips) {
        ...
        showToast('تعذر تحميل الرحلات الآن. حاول مرة تانية بعد قليل.', 'error');
        return;
      }
  ...
  const pageKey = `${activeView}:${activeTab}:${viewedTicket?.bookingId || viewedTicket?.id || currentInvoice?.invoiceNumber || 'base'}`;
  ...
  return (
    <>
      ...
      <aside
        className={`sidebar-shell hidden h-full shrink-0 flex-col border-l border-slate-200 bg-[linear-gradient(180deg,rgba(255,255,255,0.94)_0%,rgba(248,250,252,0.92)_100%)] px-4 py-5 backdrop-blur dark:border-slate-800 dark:bg-[linear-gradient(180deg,rgba(2,6,23,0.98)_0%,rgba(15,23,42,0.94)_100%)] md:flex ${
          isSidebarOpen ? 'w-[312px]' : 'w-[104px]'
        }`}
      >
        <div className={`flex items-center ${isSidebarOpen ? 'justify-between gap-3' : 'justify-center'}`}>
          <button
            ...
            className={`interactive-press flex items-center ${isSidebarOpen ? 'gap-3' : 'justify-center'} text-right`}
          >
            <span className="grid h-14 w-14 place-items-center rounded-[26px] bg-[linear-gradient(135deg,#163c98_0%,#2156d9_100%)] text-white shadow-[0_18px_34px_-18px_rgba(33,86,217,0.55)]">
            ...
            {isSidebarOpen ? (
              <span className="sidebar-panel">
                ...
                <span className="mt-1 block text-xs font-black text-slate-400 dark:text-slate-500">حجز أوضح، ورحلتك في مكانها الصح</span>
              </span>
            ) : null}
          </button>
          <button
            ...
            className={`interactive-press grid h-11 ${isSidebarOpen ? 'w-11' : 'mt-4 w-full'} place-items-center rounded-2xl border border-slate-200 bg-white text-slate-500 transition hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-slate-800`}
            aria-label={isSidebarOpen ? 'تصغير الشريط الجانبي' : 'توسيع الشريط الجانبي'}
          >
        ...
        {isSidebarOpen ? (
          latestTrip ? (
            <div className="sidebar-panel mt-6 rounded-[30px] border border-slate-200 bg-white/92 p-4 shadow-[0_20px_44px_-30px_rgba(16,35,63,0.22)] dark:border-slate-800 dark:bg-slate-900/92">
              <p className="text-xs font-black tracking-[0.16em] text-slate-400 dark:text-slate-500">الأقرب ليك الآن</p>
              <p className="mt-3 text-lg font-black text-slate-900 dark:text-white">{latestTrip.from} → {latestTrip.to}</p>
              <p className="mt-1 text-sm font-bold text-slate-500 dark:text-slate-400">{latestTrip.date} · {latestTrip.departureTime}</p>
              <div className="mt-4 flex flex-wrap gap-2">
                <MetaChip label={`المقعد ${Array.isArray(latestTrip.seatNumbers) ? latestTrip.seatNumbers.join('، ') : '—'}`} tone="brand" />
                <MetaChip label={latestTrip.pnr ? `PNR ${latestTrip.pnr}` : 'تذكرة جاهزة'} tone="neutral" />
              </div>
              <div className="mt-4 flex gap-2">
                <button
                  ...
                  className="interactive-press inline-flex flex-1 items-center justify-center rounded-2xl bg-indigo-600 px-4 py-3 text-sm font-black text-white shadow-[0_20px_36px_-24px_rgba(33,86,217,0.65)] transition hover:bg-indigo-700"
                >
                  افتح التذكرة
                </button>
                <button
                  type="button"
                  onClick={() => navigateTo('main', 'bookings')}
                  className="interactive-press inline-flex items-center justify-center rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-black text-slate-700 transition hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200 dark:hover:bg-slate-800"
                >
                  رحلاتي
                </button>
              </div>
            ...
            <div className="sidebar-panel mt-6 rounded-[30px] border border-slate-200 bg-white/92 p-4 shadow-[0_20px_44px_-30px_rgba(16,35,63,0.22)] dark:border-slate-800 dark:bg-slate-900/92">
              ...
              <p className="mt-4 text-sm font-bold leading-6 text-slate-500 dark:text-slate-400">ابدأ بحث جديد من الرئيسية، وكل ما يخص الرحلات والتذاكر هتلاقيه في الصفحات المخصصة ليه.</p>
            </div>
          )
        ) : null}
        ...
        <nav className="mt-6 flex-1 space-y-2">
          ...
          <DesktopNavItem icon={<BusFront />} label="رحلاتي" active={activeTab === 'bookings'} onClick={() => navigateTo('main', 'bookings')} collapsed={!isSidebarOpen} />
        ...
        {isSidebarOpen ? (
          <div className="sidebar-panel mt-6 grid grid-cols-2 gap-2">
            <button type="button" onClick={() => setActiveModal('notifications')} className="interactive-press rounded-[22px] border border-slate-200 bg-white px-4 py-3 text-right text-sm font-black text-slate-700 transition hover:border-indigo-200 hover:text-indigo-700 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-200 dark:hover:border-indigo-800 dark:hover:text-indigo-300">
              التنبيهات {unreadCount ? `(${unreadCount})` : ''}
            </button>
            <button type="button" onClick={openGuide} className="interactive-press rounded-[22px] border border-slate-200 bg-white px-4 py-3 text-right text-sm font-black text-slate-700 transition hover:border-indigo-200 hover:text-indigo-700 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-200 dark:hover:border-indigo-800 dark:hover:text-indigo-300">
              الدليل
            </button>
          </div>
        ) : null}
      ...
      <div className="relative flex min-w-0 flex-1 flex-col bg-[var(--bg)] dark:bg-slate-950">
        <div className="pointer-events-none absolute inset-0 opacity-30" style={{ backgroundImage: 'radial-gradient(circle at top right, rgba(33,86,217,0.10), transparent 24%), radial-gradient(circle at bottom left, rgba(15,159,138,0.08), transparent 20%), linear-gradient(rgba(15,23,42,0.03) 1px, transparent 1px), linear-gradient(90deg, rgba(15,23,42,0.03) 1px, transparent 1px)', backgroundSize: 'auto, auto, 24px 24px, 24px 24px' }} />

        <header className="sticky top-0 z-40 border-b border-slate-200/70 bg-white/88 px-4 py-4 backdrop-blur dark:border-slate-800/80 dark:bg-slate-950/84 md:px-6">
        ...
        <main className="relative flex-1 overflow-y-auto">
          <div className="mx-auto flex min-h-full w-full max-w-[1800px] flex-col px-4 pb-28 pt-4 md:px-6 md:pb-8 md:pt-6 xl:px-8"><div key={pageKey} className="page-enter">
              ...
              <SearchResultsView
                ...
                onSelectTrip={async (trip) => {
                  try {
                    ...
                    showToast('تعذر تحميل المقاعد الآن.', 'error');
                  }
                }}
                showToast={showToast}
        ...
        {activeView === 'main' ? (
          <div className="fixed inset-x-0 bottom-0 z-40 px-4 pb-[calc(env(safe-area-inset-bottom)+12px)] md:hidden">
            <nav className="mx-auto flex max-w-[420px] items-center justify-around rounded-[30px] border border-slate-200/80 bg-white/94 p-2 shadow-[0_20px_45px_-28px_rgba(16,35,63,0.28)] backdrop-blur dark:border-slate-800 dark:bg-slate-950/90">
              <BottomNavItem icon={<Home />} label="الرئيسية" active={activeTab === 'home'} onClick={() => navigateTo('main', 'home')} />
              <BottomNavItem icon={<BusFront />} label="رحلاتي" active={activeTab === 'bookings'} onClick={() => navigateTo('main', 'bookings')} />
              <BottomNavItem icon={<Ticket />} label="التذاكر" active={activeTab === 'tickets'} onClick={() => navigateTo('main', 'tickets')} />
