...
import { AppSurface, MetaChip } from './ui/AppPrimitives';
...
const buildInvoiceItems = ({ passengers, baseTotal, luggageFee, autoDiscount, promoDiscount }) => [
...
function SidebarNavButton({ item, active, compact, onClick }) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`app-pressable group relative flex w-full items-center gap-3 rounded-[22px] px-3 py-3 text-right transition-all duration-300 ${compact ? 'justify-center px-0' : ''} ${active ? 'bg-[linear-gradient(135deg,#163c98_0%,#2156d9_100%)] text-white shadow-[0_22px_48px_-26px_rgba(16,35,63,0.65)]' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-950 dark:text-slate-300 dark:hover:bg-slate-800/90 dark:hover:text-white'}`}
    >
      <span className={`grid h-11 w-11 place-items-center rounded-[16px] transition-all duration-300 ${active ? 'bg-white/12 text-white ring-1 ring-white/10' : 'bg-slate-100 text-slate-600 group-hover:bg-white dark:bg-slate-800 dark:text-slate-300 dark:group-hover:bg-slate-700'}`}>
  ...
  return (
    <>
      ...
      <div className="flex min-h-[100dvh] w-full bg-[var(--bg)] text-slate-900 dark:bg-slate-950 dark:text-slate-50">
        <aside className={`app-shell-glow app-sidebar-surface app-page-transition fixed inset-y-0 right-0 z-40 hidden border-l border-white/10 px-4 py-5 md:flex ${isSidebarCompact ? 'w-[108px]' : 'w-[312px]'}`}>
          <div className="flex h-full w-full flex-col gap-5">
            <div className={`flex items-center ${isSidebarCompact ? 'justify-center' : 'justify-between gap-3'}`}>
              ...
              >
                <span className="grid h-12 w-12 place-items-center rounded-[18px] bg-white/12 text-white shadow-[0_18px_45px_-25px_rgba(16,35,63,0.45)] ring-1 ring-white/10 dark:bg-white/8">
                  <Compass className="h-5 w-5" />
                </span>
                {!isSidebarCompact ? (
                  <div>
                    <p className="text-lg font-black text-white dark:text-white">طريقي</p>
                    <p className="text-xs font-bold text-slate-300 dark:text-slate-400">رحلات مصر بشكل أوضح</p>
                  </div>
                ) : null}
              </button>
              {!isSidebarCompact ? (
                <button
                  ...
                  className="app-pressable grid h-11 w-11 place-items-center rounded-[18px] border border-white/10 bg-white/6 text-slate-200 transition hover:bg-white/10"
            ...
            {isSidebarCompact ? (
              <button
                ...
                className="app-pressable mx-auto grid h-11 w-11 place-items-center rounded-[18px] border border-white/10 bg-white/6 text-slate-200 transition hover:bg-white/10"
            ...
            <div className={`space-y-3 ${isSidebarCompact ? 'items-center' : ''}`}>
              {!isSidebarCompact ? (
                <div className="rounded-[24px] border border-white/10 bg-white/6 p-4 text-slate-100 backdrop-blur-sm">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <p className="text-xs font-black tracking-[0.16em] text-slate-300">الرصيد الحالي</p>
                      <p className="mt-2 text-xl font-black">{wallet} ج.م</p>
                    </div>
                    <MetaChip label={`${Number(points || 0)} نقطة`} tone="neutral" className="border-white/10 bg-white/10 text-white dark:border-white/10 dark:bg-white/10 dark:text-white" />
                  </div>
                </div>
              ) : null}
              ...
              <button
                ...
                className={`app-pressable flex w-full items-center gap-3 rounded-[22px] border px-3 py-3 transition-all duration-300 ${isSidebarCompact ? 'justify-center px-0' : ''} border-white/10 bg-white/6 text-slate-100 hover:bg-white/10`}
              >
                <span className="grid h-11 w-11 place-items-center rounded-[16px] bg-white/10 ring-1 ring-white/10">
        ...
        {isMobileMenuOpen ? (
          <div className="fixed inset-0 z-50 bg-slate-950/55 backdrop-blur-sm md:hidden" onClick={() => setIsMobileMenuOpen(false)}>
            <div className="h-full w-[84vw] max-w-[340px] border-l border-slate-800 bg-slate-950 p-4" onClick={(event) => event.stopPropagation()}>
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-lg font-black text-white">طريقي</p>
                  <p className="text-xs font-bold text-slate-400">تنقل أوضح وأسهل</p>
                </div>
                <button type="button" onClick={() => setIsMobileMenuOpen(false)} className="grid h-11 w-11 place-items-center rounded-[18px] border border-white/10 bg-white/6 text-white">
                  <X className="h-5 w-5" />
                </button>
              </div>
              <div className="mt-6 space-y-2">
                ...
                <button type="button" onClick={() => setActiveModal('help')} className="app-pressable mt-2 flex w-full items-center gap-3 rounded-[22px] px-3 py-3 text-right text-slate-300 transition hover:bg-slate-800 hover:text-white">
                  <span className="grid h-11 w-11 place-items-center rounded-[16px] bg-slate-800 text-slate-200"><HelpCircle className="h-5 w-5" /></span>
        ...
        <div className={`flex min-h-[100dvh] flex-1 flex-col ${isSidebarCompact ? 'md:pr-[108px]' : 'md:pr-[312px]'}`}>
          ...
          {activeView === 'main' ? (
            <div className="fixed inset-x-0 bottom-0 z-40 px-4 pb-[calc(env(safe-area-inset-bottom)+12px)] md:hidden">
              <nav className="mx-auto grid max-w-[430px] grid-cols-5 items-center gap-2 rounded-[28px] border border-slate-200/80 bg-white/92 p-2 shadow-[0_22px_50px_-28px_rgba(16,35,63,0.35)] backdrop-blur-xl dark:border-slate-800 dark:bg-slate-950/92">
                {NAV_ITEMS.map((item) => {
                  const Icon = item.icon;
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      onClick={() => navigateTo('main', item.key)}
                      className={`app-pressable flex flex-col items-center gap-1 rounded-[20px] px-2 py-2 text-[11px] font-black transition-all ${active ? 'bg-slate-950 text-white dark:bg-white dark:text-slate-950' : 'text-slate-500 dark:text-slate-400'}`}
                    >
      ...
      {internalOpsEnabled ? (
        <InternalOpsPanel
          ...
          setViewedTicket={setViewedTicket}
          handleSearch={handleSearch}
          createBookingForTrip={createBookingForTrip}
          finalizeBookingSuccess={finalizeBookingSuccess}
          commitSeatSelection={commitSeatSelection}
          hydrateTripWithSeats={hydrateTripWithSeats}
          getLocalDateInputValue={getLocalDateInputValue}
        />
      ) : null}
    </>
  );
