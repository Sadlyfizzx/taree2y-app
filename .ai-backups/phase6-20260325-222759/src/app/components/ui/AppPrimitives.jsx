...
export function AppSurface({ children, className = '', tone = 'default' }) {
  return (
    <section
      className={cx(
        'surface-pop rounded-[28px] border border-slate-200/80 bg-white shadow-[0_18px_40px_-28px_rgba(16,35,63,0.24)] transition-[transform,box-shadow,border-color,background-color] duration-300 hover:shadow-[0_24px_52px_-32px_rgba(16,35,63,0.28)] dark:border-slate-800 dark:bg-slate-900',
...
export function PrimaryButton({
  ...
  return (
    <button
      type="button"
      className={cx(
        'inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl bg-indigo-600 px-5 py-3 text-sm font-black text-white shadow-lg shadow-indigo-600/25 transition-all duration-200 hover:-translate-y-[1px] hover:bg-indigo-700 hover:shadow-indigo-600/35 active:translate-y-0 active:scale-[0.985] disabled:cursor-not-allowed disabled:bg-slate-300 disabled:text-slate-500 disabled:shadow-none dark:disabled:bg-slate-800 dark:disabled:text-slate-500',
        className,
      )}
...
export function SecondaryButton({
  ...
  return (
    <button
      type="button"
      className={cx(
        'inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white px-5 py-3 text-sm font-black text-slate-700 transition-all duration-200 hover:-translate-y-[1px] hover:border-indigo-200 hover:bg-indigo-50 active:translate-y-0 active:scale-[0.985] disabled:cursor-not-allowed disabled:opacity-60 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:border-indigo-700 dark:hover:bg-slate-800',
        className,
      )}
      disabled={resolvedDisabled}
      aria-busy={loading || undefined}
...
export function DesktopNavItem({ icon, label, active, onClick, collapsed }) {
  return (
    <button
      ...
      className={cx(
        'interactive-press desktop-glow group relative flex w-full items-center rounded-[24px] border px-4 py-3.5 text-right transition-all duration-300',
        collapsed ? 'justify-center px-3' : 'gap-3',
        active
          ? 'border-indigo-200 bg-[linear-gradient(135deg,rgba(255,255,255,0.98)_0%,rgba(238,244,255,1)_100%)] text-indigo-700 shadow-[0_18px_30px_-24px_rgba(33,86,217,0.5)] dark:border-indigo-900/50 dark:bg-indigo-900/20 dark:text-indigo-300'
          : 'border-transparent text-slate-600 hover:border-slate-200 hover:bg-white/90 hover:text-slate-900 dark:text-slate-300 dark:hover:border-slate-800 dark:hover:bg-slate-900/90 dark:hover:text-white',
      )}
    >
      {active ? <span className="absolute inset-y-3 right-1 w-1 rounded-full bg-indigo-600 dark:bg-indigo-400" /> : null}
      <span className={cx(
        'grid h-11 w-11 shrink-0 place-items-center rounded-2xl transition-all',
        active ? 'bg-indigo-50 text-current dark:bg-indigo-900/30' : 'bg-white/80 text-current dark:bg-slate-800/80',
      )}>
        {React.cloneElement(icon, { className: 'h-5 w-5' })}
      </span>
      {!collapsed ? (
...
export function BottomNavItem({ icon, label, active, onClick }) {
  return (
    <button
      ...
      aria-current={active ? 'page' : undefined}
      className={cx(
        'interactive-press group relative flex min-h-[60px] flex-1 flex-col items-center justify-center gap-1 rounded-[22px] px-2 text-center transition-all duration-200',
        active ? 'bg-indigo-50/80 text-indigo-700 dark:bg-indigo-900/20 dark:text-indigo-300' : 'text-slate-400 dark:text-slate-500',
      )}
    >
      {active ? (
