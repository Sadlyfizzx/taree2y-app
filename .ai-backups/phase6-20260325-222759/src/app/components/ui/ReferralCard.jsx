...
export default function ReferralCard({
  ...
  const backendUnavailable = !summary && !referralCode && !appliedCode;
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      showToast('تعذر تحديث حالة الدعوات حالياً.', 'error');
    } finally {
      setBusyAction('');
    }
  };
  ...
  return (
    <AppSurface className={`p-6 ${className}`.trim()}>
      <SectionHeader
        ...
        subtitle="شارك كودك أو اربط حسابك بكود دعوة قبل أول حجز ناجح."
      />

      {backendUnavailable ? (
        <div className="mt-5">
          <InlineNotice tone="neutral" title="ميزة الدعوات غير جاهزة على هذا الحساب الآن" text="الواجهة جاهزة، لكن بيانات الدعوات لم تصل من السيرفر بعد. أول ما تتوفر هتظهر هنا مباشرة." />
        </div>
      ) : null}
      ...
      <div className="mt-5 grid gap-4 xl:grid-cols-[1.05fr_0.95fr]">
        <div className="rounded-[28px] border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-2 text-sm font-bold leading-6 text-slate-500 dark:text-slate-400">
                شاركه مع أصحابك، وكل دعوة ناجحة هتظهر حالتها هنا بشكل واضح.
              </p>
            </div>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <MetaChip label={`${safeNumber(summary?.pendingInvites)} بانتظار أول رحلة`} tone="warning" />
            <MetaChip label={`${safeNumber(summary?.qualifiedInvites)} متفعلة`} tone="success" />
            <MetaChip label={`${safeNumber(summary?.rewardedInvites)} مكافأة مكتملة`} tone="neutral" />
          </div>
        ...
        <div className="rounded-[28px] border border-slate-200 bg-slate-50 p-5 dark:border-slate-800 dark:bg-slate-950/60">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-2 text-sm font-bold leading-6 text-slate-500 dark:text-slate-400">
                لو جالك كود من صديق، استخدمه مرة واحدة قبل أول حجز ناجح.
              </p>
            </div>
