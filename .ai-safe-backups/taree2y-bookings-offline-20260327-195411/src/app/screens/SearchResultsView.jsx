...
function SearchResultsView({
  ...
  const displayedTrips = useMemo(() => {
    ...
    const sortSoldOutLast = (tripA, tripB) => {
      const soldOutA = availabilityValue(tripA) === 0 ? 1 : 0;
      const soldOutB = availabilityValue(tripB) === 0 ? 1 : 0;
  ...
  const soldOutTripsCount = useMemo(
    () =>
      displayedTrips.filter((trip) => {
        const availableSeats = Number.isFinite(Number(trip.availableSeatsCount))
          ? Number(trip.availableSeatsCount)
          : trip.seats?.filter((seat) => seat.status === 'available').length || 0;
  ...
  const availabilityNotice = useMemo(() => {
    ...
    if (totalAvailableSeats <= 30) {
      return {
        ...
        text: 'قارن الوقت والمحطة والسعر براحتك، لكن بعض الرحلات بدأت تتملي.',
      };
    }
    ...
    return {
      ...
      text: 'قدامك أكتر من اختيار، فركز على المحطة المناسبة والوقت والسعر.',
    };
