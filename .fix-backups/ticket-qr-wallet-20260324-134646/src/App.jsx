...
import PublicTrackingView from './app/screens/PublicTrackingView';
import { readPublicTripPayloadFromLocation } from './app/utils/publicTripSharing';
...
export default function App() {
  const publicTripPayload = readPublicTripPayloadFromLocation();
  const { session, profile, authLoading } = useAuthSession();
  const { isDark, setIsDark } = useDarkMode();

  if (publicTripPayload) {
    return (
      <div className={`min-h-[100dvh] w-full ${isDark ? 'dark' : ''}`} dir="rtl">
        <PublicTrackingView payload={publicTripPayload} />
      </div>
    );
  }
