...
import { buildWalletTopUpLink, generateWalletTopupRequestId } from '../utils/walletTopup';
...
function WalletView({
  ...
  const topupRequestId = useMemo(() => generateWalletTopupRequestId(userId), [userId]);
  const walletTopUpLink = useMemo(
    () => buildWalletTopUpLink({ userId, amount: 100, requestId: topupRequestId }),
    [topupRequestId, userId],
  );

  const openQrTopup = () => {
    window.open(walletTopUpLink, '_blank', 'noopener,noreferrer');
  };

  const copyWalletTopupLink = async () => {
    try {
      await copyTextToClipboard(walletTopUpLink);
      showToast('تم نسخ رابط شحن المحفظة.', 'success');
    } catch {
      showToast('تعذر نسخ رابط شحن المحفظة حالياً.', 'error');
    }
  };
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="relative overflow-hidden rounded-[36px] bg-[linear-gradient(135deg,#10233f_0%,#163c98_48%,#2156d9_100%)] p-6 text-white shadow-[0_30px_60px_-36px_rgba(16,35,63,0.6)]">
        ...
        <div className="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            ...
            <SecondaryButton onClick={openQrTopup} icon={<QrCode className="h-5 w-5" />} className="border-white/20 bg-white/10 text-white hover:bg-white/15 dark:border-white/20 dark:bg-white/10 dark:text-white">
              QR الشحن
            </SecondaryButton>
          </div>
        </div>
      ...
      <div className="grid gap-5 xl:grid-cols-[0.95fr_1.05fr]">
        <PrettyQrCard
          value={walletTopUpLink}
          title="QR شحن المحفظة"
          subtitle="امسح الكود من أي جهاز لفتح صفحة شحن مرتبطة بنفس الحساب."
          chipLabel={topupRequestId}
          codeLabel="رابط الشحن"
          className="h-full"
        />
        ...
        <div className="space-y-4">
          ...
          <AppSurface className="p-5">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-sm font-black text-slate-900 dark:text-white">رابط الشحن من أي جهاز</p>
                <p className="mt-1 text-sm font-bold text-slate-500 dark:text-slate-400">يفتح نفس الحساب مباشرة ويضيف الرصيد له.</p>
              </div>
              <div className="flex flex-col gap-3 sm:flex-row">
                <PrimaryButton onClick={openQrTopup}>افتح رابط الشحن</PrimaryButton>
                <SecondaryButton onClick={copyWalletTopupLink}>انسخ الرابط</SecondaryButton>
              </div>
            </div>
          </AppSurface>
        </div>
