...
export default function PublicWalletTopUpView() {
  ...
  const isReady = Boolean(userId && requestId && amount >= 10);
  ...
  const handleConfirm = async () => {
    ...
    try {
      await publicTopupWallet({ userId, amount, requestId });
      setStatus('success');
      setMessage('تم تأكيد الشحن بنجاح. ارجع للتطبيق وستجد الرصيد مضافاً لنفس الحساب.');
    } catch (error) {
      setStatus('error');
      ...
      <section className="relative overflow-hidden rounded-[36px] bg-[linear-gradient(135deg,#10233f_0%,#163c98_48%,#2156d9_100%)] p-6 text-white shadow-[0_30px_60px_-36px_rgba(16,35,63,0.6)]">
        ...
        <div className="relative z-10 flex items-start justify-between gap-4">
          <div>
            ...
            <h1 className="mt-3 text-3xl font-black">{formatCurrency(amount)}</h1>
            <p className="mt-2 text-sm font-bold text-white/80">سيتم إضافة الرصيد مباشرة لنفس الحساب بعد التأكيد.</p>
          </div>
      ...
      <AppSurface className="p-5">
        <div className="space-y-4">
          ...
          <div className="rounded-[24px] border border-slate-200 bg-slate-50 px-4 py-4 dark:border-slate-800 dark:bg-slate-950/60">
            ...
            <p className="mt-2 text-sm font-bold leading-6 text-slate-500 dark:text-slate-400">يمكنك فتح الصفحة من أي جهاز، وسيتم إضافة الرصيد إلى محفظة نفس المستخدم.</p>
          </div>
