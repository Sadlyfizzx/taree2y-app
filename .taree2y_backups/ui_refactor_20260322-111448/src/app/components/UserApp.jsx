...
export default function UserApp({ userId, profile, isDark, setIsDark }) {
  ...
  const createDemoBookingResult = ({ trip, seatNumbers, passengers, promoCode, hasLuggage, rideToStation, needsAccess }) => {
    ...
    const pointsToAwardLater = Math.max(0, Math.floor(Math.max(0, baseTotal - autoDiscount - promoDiscount) / 5));
    ...
    const invoice = {
      ...
      items: buildInvoiceItems({ passengers, baseTotal, luggageFee, rideFee, autoDiscount, promoDiscount }),
    };
      ...
      <div className="flex-1 flex flex-col relative h-full bg-slate-50 dark:bg-slate-950 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto relative hide-scrollbar scroll-smooth flex flex-col w-full">
          <div className="w-full mx-auto max-w-[1800px] flex-1 flex flex-col pb-32 md:pb-8 px-0 lg:px-8">
            ...
            {activeView === 'checkout' && selectedTrip && <CheckoutView trip={selectedTrip} seats={selectedSeats} passengers={searchParams.passengers} wallet={wallet} subscription={subscription} onCreateBooking={createBookingForTrip} onSuccess={finalizeBookingSuccess} showToast={showToast} openModal={setActiveModal} />}
