...
const generateTrips = (from, to, date) => {
  ...
  const trips = SCHEDULE_TEMPLATES.map((schedule, i) => {
    const isVIP = schedule.class.includes('VIP');
    const durationMin = Math.max(120, Math.round((basePrice / 60) * 60 + (rand() * 40 - 20)));
    const price = basePrice + (isVIP ? Math.floor(basePrice * 0.5) : 0) + Math.floor(rand() * 3) * 10;
    ...
    return {
      ...
      seats: generateSeats(tripId, date, schedule.departureTime),
...
function CheckoutView({ trip, seats, passengers, wallet, setWallet, setTransactions, setPoints, subscription, onSuccess, showToast, openModal }) {
  ...
  const handlePayment = () => {
    ...
    setTimeout(() => {
      ...
      const ticket = { ...trip, pnr, bookingDate: getLocalDateInputValue(), selectedSeats: seats, finalTotal, luggage: hasLuggage, ride: rideToStation, access: needsAccess, paymentMethod: 'wallet', status: 'upcoming' };
...
function TripsView({ trips, setMyTrips, processRefund, onViewTicket, showToast }) {
  ...
  const computedTrips = useMemo(() => {
    ...
    return trips.map((t) => {
      ...
      const arrival = new Date(`${t.date}T${t.arrivalTime}:00`);
      if (now > arrival) return { ...t, status: 'past' };
...
function TrackingView({ ticket, showToast, openModal }) {
  ...
  const departureDate = new Date(`${ticket.date}T${ticket.departureTime}:00`);
  ...
  if (isPast || now > arrivalDate) {
     ...
     statusText = 'الرحلة شغالة حسب الجدول';
     isMoving = true;
  }
