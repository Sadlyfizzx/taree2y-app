...
const generateSeats = (tripId, tripDate, tripTime) => {
  ...
  let bookedProb = hoursDiff < 0 ? 1.0 : hoursDiff < 12 ? 0.90 : hoursDiff < 48 ? 0.70 : hoursDiff < 120 ? 0.40 : 0.15;
  const randomSeed = (tripId.charCodeAt(tripId.length - 1) % 10) / 100;

  for (let i = 1; i <= 40; i++) {
    const isBooked = Math.random() < (bookedProb + randomSeed);
    seats.push({ id: i, number: `${i}${['A','B','C','D'][i%4]}`, status: isBooked ? 'booked' : 'available' });
  }
  return seats;
};
...
const generateTrips = (from, to, date) => {
  ...
  const numTrips = Math.floor(Math.random() * 4) + 4; 
  const trips = Array.from({ length: numTrips }).map((_, i) => {
    const id = `TRP-${Math.random().toString(36).substr(2, 6).toUpperCase()}`;
    const hour = 5 + (i * 2) + Math.floor(Math.random() * 2); 
    const timeStr = `${hour.toString().padStart(2, '0')}:00`;
    const isVIP = i % 2 === 0;
    
    const baseDuration = Math.max(2, basePrice / 60); 
    const randomVariance = (Math.random() * 1.5) - 0.5; 
    const finalDuration = baseDuration + randomVariance;
    
    const arrHour = Math.floor(hour + finalDuration) % 24;
    const arrMins = Math.round((finalDuration % 1) * 60);
    const arrivalStr = `${arrHour.toString().padStart(2, '0')}:${arrMins.toString().padStart(2, '0')}`;
    
    let finalPrice = isVIP ? Math.floor(basePrice * 1.5) : basePrice;
    finalPrice += Math.floor(Math.random() * 4) * 10; 

    return {
      id, from, to, date, departureTime: timeStr, arrivalTime: arrivalStr,
      durationHour: parseFloat(finalDuration.toFixed(1)), price: finalPrice,
      company: ['جو باص', 'سوبر جيت', 'بلو باص'][i % 3], class: isVIP ? 'VIP رجال أعمال' : 'اقتصادي مميز',
      rating: (Math.random() * (5.0 - 4.0) + 4.0).toFixed(1), seats: generateSeats(id, date, timeStr),
  ...
  return { 
    trips: trips.map(t => ({ ...t, badge: t.id === cheapest?.id ? 'cheapest' : t.id === fastest?.id ? 'fastest' : t.class.includes('VIP') ? 'vip' : null })),
    isDirect: true 
  };
};
...
function UserApp({ userId, profile, onLogout, isDark, setIsDark }) {
  ...
  const processDelayedRefund = (tripToCancel) => {
    ...
    const refundAmount = tripToCancel.finalTotal * (1 - feeRatio);
    ...
    setTimeout(() => {
      setMyTrips(currentTrips => {
         ...
         if (exists) {
            ...
            setTransactions(p => [{ id: `REF-${Math.random().toString(36).substr(2,4).toUpperCase()}`, type: 'credit', amount: refundAmount, date: new Date().toISOString().split('T')[0], desc: `استرداد تذكرة ${tripToCancel.pnr}` }, ...p]);
            showToast(`تم الإلغاء! رجعلك ${refundAmount} ج.م للمحفظة 💸`, 'success');
            return currentTrips.map(t => t.pnr === tripToCancel.pnr ? {...t, status: 'cancelled'} : t);
         }
         return currentTrips;
      });
    }, 5000); 
        ...
        {isSearching ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
             {displayedTrips.map((trip) => {
               ...
               return (
               <div key={trip.id} onClick={() => availableSeats > 0 ? onSelectTrip(trip) : showToast('سجلنا اسمك في قائمة الانتظار، هنبلغك لو في مكان فضي ⏳', 'success')} 
    ...
    <div className="flex flex-col flex-1 pt-4 w-full h-full">
      ...
      <div className="flex-1 overflow-y-auto px-5 hide-scrollbar shrink-0 flex items-center justify-center">
        <div className="bg-white dark:bg-slate-800 rounded-[3rem] p-6 max-w-[320px] mx-auto border-4 border-slate-200 dark:border-slate-700 relative shadow-sm mb-6">
          ...
          <div className="grid grid-cols-4 gap-3 gap-x-8" dir="ltr">
...
function CheckoutView({ trip, seats, passengers, wallet, setWallet, setTransactions, setPoints, subscription, onSuccess, showToast, openModal }) {
  ...
  const handlePayment = () => {
    ...
    setTimeout(() => {
      ...
      const earnedPoints = Math.floor(finalTotal / 5);
      ...
      <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-sm border border-slate-100 dark:border-slate-700 shrink-0">
         ...
         <div className="space-y-3 text-sm font-bold">
            ...
            <div className="text-[10px] text-center text-slate-400 bg-slate-50 dark:bg-slate-900 py-1.5 rounded-lg flex items-center justify-center gap-1">
               <Award className="w-3 h-3 text-indigo-500"/> هتكسب {Math.floor(finalTotal/5)} نقطة ولاء بعد الرحلة دي
            </div>
         </div>
      </div>
  ...
  useEffect(() => {
    ...
    const updatedTrips = trips.map(t => {
      if (t.status === 'upcoming') {
        const tripDate = new Date(`${t.date}T${t.arrivalTime.split(':')[0]}:00`);
        if (now > tripDate) { changed = true; return { ...t, status: 'past' }; }
      }
      return t;
    });
...
function TrackingView({ ticket, showToast, openModal }) {
  ...
  const departureDate = new Date(`${ticket.date}T${ticket.departureTime.split(':')[0]}:00`);
  const [arrH, arrM] = ticket.arrivalTime.split(':');
  const arrivalDate = new Date(`${ticket.date}T${arrH}:${arrM}:00`);
...
function TopUpFlowModal({ closeModal, wallet, setWallet, setTransactions, showToast }) {
  ...
  const handleSendOTP = () => {
     ...
     showToast('بعتنالك الكود ✉️', 'success');
     setTimeout(() => setOtp('123456'), 1500); 
  };
           ...
           <>
            ...
            {method === 'vodafone' && (
               <div className="space-y-4">
                  ...
                  {!otp && <button onClick={handleSendOTP} className="w-full bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-white font-bold py-3 rounded-xl">إرسال كود التأكيد (OTP)</button>}
                  {otp && (
                     <div className="animate-fade-in-down space-y-2">
                        ...
                        <input type="text" value={otp} onChange={e=>setOtp(e.target.value)} className="w-full bg-emerald-50 dark:bg-emerald-900/20 border-2 border-emerald-500 rounded-2xl py-4 px-4 text-center text-xl tracking-[0.5em] font-black text-emerald-700 dark:text-emerald-400 outline-none" dir="ltr" />
                     </div>
                  )}
               </div>
            )}
