...
function UserApp({ userId, profile, onLogout, isDark, setIsDark }) {
  ...
  const refreshCloudState = useCallback(async ({ silent = false } = {}) => {
    if (!userId) return;
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(async () => {
      try {
        await saveSupabaseAppState(userId, {
          wallet,
          transactions,
          myTrips,
          points,
    ...
    }, didSeedSupabaseFromLocal ? 100 : 300);

    return () => clearTimeout(timeoutId);
  }, [userId, wallet, transactions, myTrips, points, subscription, backendReady, didSeedSupabaseFromLocal]);
          ...
          <div className="w-full mx-auto max-w-[1800px] flex-1 flex flex-col pb-32 md:pb-8 px-0 lg:px-8">
            ...
            {activeView === 'checkout' && selectedTrip && (
              <CheckoutView trip={selectedTrip} seats={selectedSeats} passengers={searchParams.passengers} wallet={wallet} setWallet={setWallet} setTransactions={setTransactions} setPoints={setPoints} subscription={subscription}
                onSuccess={(ticket, invoice) => { setMyTrips((prev) => [ticket, ...prev]); setCurrentInvoice(invoice); setViewedTicket(ticket); navigateTo('invoice'); }} showToast={showToast} openModal={setActiveModal} />
            )}
