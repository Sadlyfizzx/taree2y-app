...
export default function PromoOfferModal({ offer, closeModal, showToast, onDismiss, onCopyOffer }) {
  if (!offer) return null;

  const handleCopy = async () => {
    const copied = await copyTextWithFallback(offer.code, 'كود الخصم');

    showToast(copied ? `تم نسخ كود ${offer.code}.` : 'تعذر نسخ الكود حالياً.', copied ? 'success' : 'error');
  ...
  return (
    <ModalShell
      ...
      title="عرض مناسب لك"
      subtitle="لو مناسب لرحلتك، استخدمه في خطوة الدفع."
      ...
      footer={
        <div className="flex flex-col gap-3 sm:flex-row sm:justify-end">
          ...
          {offer.code ? <PrimaryButton onClick={handleCopy}>انسخ الكود</PrimaryButton> : null}
        </div>
      }
    >
      <div className="space-y-5">
        <div className="overflow-hidden rounded-[28px] bg-[linear-gradient(135deg,#10233f_0%,#163c98_48%,#2156d9_100%)] p-5 text-white shadow-[0_24px_60px_-28px_rgba(16,35,63,0.38)]">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-xs font-black tracking-[0.16em] text-white/70">عرض متاح الآن</p>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {offer.code ? (
              <MetaChip label={`الكود ${offer.code}`} tone="brand" className="border-white/10 bg-white/10 text-white" />
            ) : null}
            {endsAtText ? (
              <MetaChip label={`حتى ${endsAtText}`} tone="brand" className="border-white/10 bg-white/10 text-white" />
            ) : null}
          </div>
        </div>

        <div className="rounded-[24px] border border-slate-200 bg-slate-50 px-4 py-4 dark:border-slate-800 dark:bg-slate-950/60">
          <p className="text-sm font-black text-slate-900 dark:text-white">قبل الاستخدام</p>
          <ul className="mt-3 space-y-2 text-sm font-bold leading-6 text-slate-500 dark:text-slate-400">
            <li>• اكتب الكود في خطوة الدفع.</li>
            <li>• لو العرض متاح على رحلتك، الخصم هيتطبق قبل التأكيد.</li>
            <li>• لو العرض انتهى أو غير مناسب، هتعرف قبل ما تكمل.</li>
          </ul>
        </div>
      </div>
    </ModalShell>
