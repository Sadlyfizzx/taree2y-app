...
function CheckoutView({
  ...
  const finalTotalPreview = Math.max(
    0,
    baseTotal + luggageFee + rideFee - discount - autoDiscount,
  );
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (!result?.ok) {
        ...
        return showToast(
          result?.message || 'حصل خطأ أثناء تأكيد الحجز',
          'error',
        );
      }
  ...
  return (
    <div className="flex flex-col flex-1 p-5 space-y-5 max-w-xl mx-auto w-full">
      <h2 className="text-xl font-black text-slate-800 dark:text-slate-100 shrink-0">
        تأكيد ودفع 💳
      </h2>
      <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-sm border border-slate-100 dark:border-slate-700 relative overflow-hidden shrink-0">
        <div className="absolute top-1/2 -right-3 w-6 h-6 bg-slate-50 dark:bg-slate-950 rounded-full -translate-y-1/2" />
        <div className="absolute top-1/2 -left-3 w-6 h-6 bg-slate-50 dark:bg-slate-950 rounded-full -translate-y-1/2" />
        <div className="absolute top-1/2 right-4 left-4 h-px border-t-2 border-dashed border-slate-100 dark:border-slate-700 -translate-y-1/2 z-0" />
        <div className="relative z-10 pb-6">
          ...
          <div className="text-sm font-bold text-slate-500 dark:text-slate-400 text-left" dir="ltr">
        ...
        <div className="relative z-10 pt-6">
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              <span className="block text-[10px] text-slate-400 mb-1">الشركة والدرجة</span>
              <span className="font-bold text-slate-700 dark:text-slate-200">{trip.company} - {trip.class}</span>
            </div>
            <div className="text-left">
              <span className="block text-[10px] text-slate-400 mb-1">المقاعد ({passengers})</span>
              <span className="font-bold text-indigo-600 dark:text-indigo-400">{seats.join(', ')}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3 shrink-0">
        <div onClick={() => setHasLuggage(!hasLuggage)} className={`p-4 rounded-2xl border-2 flex items-center justify-between cursor-pointer transition-colors ${hasLuggage ? 'bg-indigo-50 dark:bg-indigo-900/30 border-indigo-500' : 'bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700'}`}>
          <div className="flex items-center gap-3">
            <div className={`w-6 h-6 rounded flex items-center justify-center border-2 ${hasLuggage ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-transparent border-slate-300 dark:border-slate-600'}`}>
        ...
        <div onClick={() => setRideToStation(!rideToStation)} className={`p-4 rounded-2xl border-2 flex items-center justify-between cursor-pointer transition-colors ${rideToStation ? 'bg-indigo-50 dark:bg-indigo-900/30 border-indigo-500' : 'bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700'}`}>
          <div className="flex items-center gap-3">
            <div className={`w-6 h-6 rounded flex items-center justify-center border-2 ${rideToStation ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-transparent border-slate-300 dark:border-slate-600'}`}>
              {rideToStation && <Check className="w-4 h-4" />}
            </div>
        ...
        <div onClick={() => setNeedsAccess(!needsAccess)} className={`p-4 rounded-2xl border-2 flex items-center justify-between cursor-pointer transition-colors ${needsAccess ? 'bg-emerald-50 dark:bg-emerald-900/30 border-emerald-500' : 'bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700'}`}>
          <div className="flex items-center gap-3">
            <div className={`w-6 h-6 rounded flex items-center justify-center border-2 ${needsAccess ? 'bg-emerald-600 border-emerald-600 text-white' : 'bg-transparent border-slate-300 dark:border-slate-600'}`}>
              {needsAccess && <Check className="w-4 h-4" />}
            </div>
      ...
      <div className="sticky bottom-0 mt-auto bg-gradient-to-t from-slate-50 via-slate-50 to-transparent dark:from-slate-950 dark:via-slate-950 py-4 pb-8 z-20 pointer-events-none">
        <button onClick={handlePayment} disabled={!isWalletSufficient || isProcessing || !tripBookability.canBook} className={`w-full max-w-[400px] mx-auto font-black text-lg py-4 rounded-2xl transition-all shadow-lg flex justify-center items-center gap-2 pointer-events-auto ${isWalletSufficient && !isProcessing && tripBookability.canBook ? 'bg-indigo-600 text-white shadow-indigo-600/30 active:scale-95' : 'bg-slate-300 dark:bg-slate-800 text-slate-500 cursor-not-allowed'}`}>
          {isProcessing ? <div className="w-6 h-6 border-3 border-white/30 border-t-white rounded-full animate-spin" /> : <>ادفع من المحفظة وأكد الحجز</>}
        </button>
      </div>
    </div>
  );
