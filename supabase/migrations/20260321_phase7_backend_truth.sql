...
create table if not exists public.trip_instances (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique(from_city, to_city, departure_date, departure_time, company, service_class)
);
...
begin
  ...
  if v_released > 0 then
    ...
    updated_at = now()
    where exists (
      select 1 from public.trip_seats ts where ts.trip_instance_id = ti.id
    );
  end if;
...
begin
  ...
  if not exists (select 1 from public.routes where route_key = v_route_key) then
    return jsonb_build_object('ok', false, 'message', 'No direct route');
  end if;
  ...
  select
    'TRP-' || upper(substr(md5(r.route_key), 1, 4)) || '-' || to_char(p_departure_date, 'YYYYMMDD') || '-' || replace(to_char(rt.departure_time, 'HH24MI'), ':', ''),
    r.route_key,
    r.from_city,
    r.to_city,
    p_departure_date,
...
declare
  ...
  v_pnr text := 'TRQ-' || upper(substr(replace(gen_random_uuid()::text, '-', ''), 1, 6));
  v_ticket_token text := replace(gen_random_uuid()::text, '-', '');
  v_booking_id uuid;
  v_points integer := 0;
...
begin
  begin
    ...
    if v_requested <> p_passengers then
      raise exception 'عدد المقاعد المختارة لازم يساوي عدد الركاب';
    end if;
    ...
    if (v_trip.departure_date + v_trip.departure_time) <= (timezone('Africa/Cairo', now()) + interval '120 minutes') then
    ...
    if coalesce(trim(p_promo_code), '') <> '' then
      ...
      if v_promo.first_time_only and exists (
        select 1 from public.bookings where user_id = v_user
      ) then
    ...
    values (
      ...
      p_seat_numbers,
      v_ticket_token,
      jsonb_build_object('booking_id', null, 'ticket_token', v_ticket_token)::text,
    ...
    if v_booked_count <> v_requested then
      raise exception 'انتهت مهلة حجز المقاعد أو تغيرت الإتاحة';
    end if;
    ...
    v_trip_payload := jsonb_build_object(
      ...
      'qrPayload', jsonb_build_object('booking_id', v_booking_id, 'ticket_token', v_ticket_token)::text
    );

    update public.bookings
    set trip_data = v_trip_payload,
        qr_payload = jsonb_build_object('booking_id', v_booking_id, 'ticket_token', v_ticket_token)::text,
        updated_at = now()
    ...
    v_invoice := jsonb_build_object(
      ...
      || case when p_ride_to_station then jsonb_build_array(jsonb_build_object('name', 'أوبر للمحطة', 'price', v_ride_fee)) else '[]'::jsonb end
